...
with st.container(border=True):
    ...
    uploaded_file = st.file_uploader(
        ...
        help="Upload a single SKILL.md for one skill, or a ZIP archive of a skill directory. "
             ...
             "Non-cortex-code models will only see the root SKILL.md content.",
    )
    ...
    if uploaded_file:
        ...
        if is_zip:
            ...
            st.info(
                "ZIP detected. For non-cortex-code models only the root SKILL.md will be "
                "used as system context. Sub-skills are fully resolved only for the "
                "cortex-code model via the skill stage."
            )
        else:
    ...
    with st.container(border=True):
        ...
        if _detail_rows.empty:
            ...
            for _, _r in _detail_rows.iterrows():
                _bl = get_baseline_scores(_r["QUESTION_ID"])
                if _bl:
