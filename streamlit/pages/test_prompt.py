    ...
    with st.status("Running evaluation…", expanded=True) as status:
        ...
        for model in models:
            ...
            for qid, qt, canonical_q, must_haves_q, qcat in q_iter:
                st.caption(f"- **{qid or 'custom'}**: Generating answer with :orange-badge[{model}]")
                if model == CORTEX_NATIVE:
                    ...
                    if not IS_SIS:
                        ...
                        st.caption(f"- **{qid or 'custom'}**: Running native Cortex Code via :blue-badge[SPCS]")
                ...
                def on_judge_done(idx, judge_name, _model=model, _qid=qid):
                    ...
                    st.caption(f"- **{_qid or 'custom'}**: Scored with :orange-badge[{judge_name}]")
...
with st.expander("Experiment history", expanded=True):
    ...
    if history_df.empty:
        ...
        history_df["Prompt preview"] = history_df["SYSTEM_PROMPT"].apply(
        ...
        st.dataframe(
            history_df[[
                "CREATED_AT", "Prompt preview", "Question", "CATEGORY",
                "MODEL", "Score %", "MH Pass %",
                "CORRECTNESS", "COMPLETENESS", "RECENCY", "CITATION_SCORE", "RECOMMENDATION",
            ]].rename(columns={
                "CREATED_AT": "Time",
