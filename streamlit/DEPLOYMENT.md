...
The app runs on `SYSTEM_COMPUTE_POOL_CPU` (owned by `ACCOUNTADMIN`, `USAGE` granted to `PUBLIC` so all roles have access). A dedicated `AEO_COMPUTE_POOL` has not been provisioned yet; `SYSTEM_COMPUTE_POOL_CPU` is the current workaround. When `AEO_COMPUTE_POOL` becomes available, update `compute_pool:` in `snowflake.yml` and redeploy.

The container runtime (`SYSTEM$ST_CONTAINER_RUNTIME_PY3_11`) pre-installs all required packages, so no EAI is needed. However, the runtime still requires `pyproject.toml` to be present. Swap in a minimal empty-deps version before deploying, then restore the full one for DevRel use.
