...
To contextualize the factorial results, we ran two additional baseline-only runs (all four factors OFF, 128 questions, same judge panel) using \texttt{llama4-maverick} (run 17) and \texttt{openai-gpt-5.4} (run 18) as respondent models.

\begin{table}[H]
\centering
\caption{Baseline scores (all factors OFF) for three respondent models, ordered by overall score. All three models also serve as judges in the panel-averaged scoring pipeline.}
\label{tab:model_baseline}
\begin{tabular}{lrrrr}
\toprule
\textbf{Model} & \textbf{Run} & \textbf{Score} & \textbf{Must-Have} \\
\midrule
\texttt{openai-gpt-5.4}  & 18 & \textbf{58.0\%} & 69.1\% \\
\texttt{claude-opus-4-6} &  1 & 53.2\%          & 62.7\% \\
\texttt{llama4-maverick} & 17 & 38.5\%          & 43.6\% \\
\bottomrule
\end{tabular}
\end{table}

\texttt{openai-gpt-5.4} leads at baseline (58.0\% score, 69.1\% MH), edging \texttt{claude-opus-4-6} by 4.8pp on score and 6.4pp on MH. \texttt{llama4-maverick} trails substantially at 38.5\% score and 43.6\% MH, a 19.5pp gap below the leading model. This spread confirms that model choice independently contributes to answer quality before any configuration augmentation is applied. Notably, all three models also serve as judges in the panel-averaged scoring; their baseline scores therefore reflect respondent quality uncoupled from evaluation preferences.

The full $2^4$ factorial replication across multiple respondent models remains an open item. The configuration hierarchy identified here (agentic tools dominant, self-critique counterproductive) is internally consistent for \texttt{claude-opus-4-6} but whether it holds across models is an open question.
...
\subsubsection*{Question Type to Documentation Action}
\label{sec:question_type_action}
...
\textbf{Debug} questions test documentation of failure states: error messages, diagnostic steps, and recovery procedures. Poor Debug scores indicate that troubleshooting guides, runbooks, and documented error patterns are missing or sparse.

\textbf{Compare} questions test decision guidance documentation: when to use one feature instead of another, tradeoffs, and architectural choices. Poor Compare scores indicate that decision guides and ``when to use X vs.\ Y'' pages are absent.

\textbf{Implement} questions test procedural documentation: working code examples, step-by-step tutorials, and correct API syntax. Poor Implement scores indicate that how-to guides are incomplete, missing working code, or reflect outdated APIs.

\textbf{Explain} questions test conceptual documentation: what a feature does, how it fits the platform, and why it exists. Poor Explain scores indicate that conceptual overview pages are thin or absent.
...
\begin{table}[H]
\centering
\caption{Question Type to Documentation Action: each weakest question type signals a distinct documentation gap and points to a specific remediation action.}
\label{tab:pm_action}
\small
