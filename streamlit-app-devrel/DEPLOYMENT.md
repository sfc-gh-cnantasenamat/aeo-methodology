...
| CoCo responds "Web browsing and web search tools are blocked" | SPCS compute pool on Snowhouse has no outbound internet egress by default. This is a Snowhouse network policy constraint, not a code bug. | To enable web tools: a Snowhouse sysadmin must create an EAI with a permissive network rule and grant it to the job service spec. `AEO_WEB_ACCESS_RULE` (HOST_PORT EGRESS to 0.0.0.0:443/80) was created in `DEVREL.CNANTASENAMAT_DEV` but `CREATE EXTERNAL ACCESS INTEGRATION` requires account-level CREATE INTEGRATION which `DEVREL_ADMIN_RL` does not have. |
