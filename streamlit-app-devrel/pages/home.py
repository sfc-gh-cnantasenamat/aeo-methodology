...
c6.metric("Models",      int(s["MODEL_COUNT"]))
...
with col_left:
    st.subheader(":material/group: Who")
    st.markdown(
    ...
    st.subheader(":material/science: What")
    st.markdown(
    ...
    st.subheader(":material/lightbulb: Why")
    st.markdown(
...
with col_right:
    st.subheader(":material/calendar_today: When")
    st.markdown(
    ...
    st.subheader(":material/location_on: Where")
    st.markdown(
        "Everything runs natively inside **Snowflake**:\n\n"
        f"- Questions, responses, and scores are stored in `{DB}.{SCH}`\n"
    ...
    st.subheader(":material/engineering: How")
    st.markdown(
